...
```
임계        = 0.2 × range_7
range_7    = **일봉** 최근 7봉 (high − low) 평균 (절대값) — 5 TF (1D/1W/1M/1Q/1Y) 모두 공통
today_low  = 일봉 마지막 봉의 low  (모든 TF 평가에 공통 사용)

통과 조건  = (today_low − MA10 ≤ 임계)  OR  (today_low − MA20 ≤ 임계)
```

> **임계 통일**: 이전엔 TF 별로 range_7 을 다르게 잡았는데(월봉이면 월봉 7봉, 연봉이면 연봉 7봉) 현재는 모든 TF 가 일봉 변동폭 기준 단일 임계를 사용. close 대비 약 1% 절대거리 (KR 중앙값). → 높은 TF 일수록 훨씬 빡빡한 터치 요구.
