...
- 데이터는 `data.resample.load(symbol, interval)`로 로드 (1h/1d는 raw 캐시 직접, 4h는 1h, 1w는 1d 우선 리샘플)
...
.venv/Scripts/python.exe -m backtest.engine.runner     --strategy sma_cross --symbol BTCUSDT --interval 1h     --start 2023-01-01 --params '{"fast":10,"slow":30}'

# 4H 봉, 명시적 수수료/슬리피지
.venv/Scripts/python.exe -m backtest.engine.runner     --strategy sma_cross --symbol ETHUSDT --interval 4h     --start 2022-01-01 --end 2024-12-31     --fee-bps 4 --slippage-bps 2 --init-capital 10000     --params '{"fast":20,"slow":60}'
```
