...
python -m data.fetch_bitget

# 단일 심볼
python -m data.fetch_bitget --symbol BTCUSDT

# 처음부터 다시 (캐시 무시, 지정 일자부터)
python -m data.fetch_bitget --since 2020-01-01
```
...
| 캐시 경로 | `data/cache/bitget_{SYMBOL}_1h.parquet` |
...
## 호출 절차 (Claude가 사용자에게 실행을 안내할 때)

1. 첫 실행이면 `--since 2020-01-01` 권장, 이후엔 인자 없이 (증분)
2. `python -m data.fetch_bitget [...]` 실행
3. 완료 후 `ls data/cache | wc -l`로 캐시 파일 수 확인 (≈ Bitget USDT-M 선물 종목 수)
