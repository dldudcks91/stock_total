...
def cell_stats(grp: pd.DataFrame, h: int = 168, col_suffix: str = "ma10") -> dict:
...
def main():
    ...
    def add_args(ap):
        ap.add_argument("--events", type=Path, default=None,
                        help="path to events.parquet (defaults to <out_dir>/events.parquet)")

    out_dir, params, args = parse_args(add_args, {}, "full_grid")
    events_path = args.events or (out_dir / "events.parquet")
    if not events_path.exists():
        # fallback: same run dir's output, named angle_study events
        alt = out_dir / "events.parquet"
        if alt.exists():
            events_path = alt
        else:
            raise SystemExit(f"events parquet not found: {events_path}")
    ...
    untouched = events[~events["touched_ma10"]].copy()
    print(f"MA10 UNtouched within 10: {len(untouched)} (fwd = fr i+10 close)")

    ev = events[events["touched_ma10"]].copy()
    ...
    def bars_label(b):
        ...
        return None
    ev["_bbin"] = ev["bars_to_touch_ma10"].apply(bars_label)

    # angle quantile per bars-group (global across all W groups in that bars range)
    angle_q_by_bars = {}
    for lo, hi in BARS_BINS:
        sub = ev[(ev["bars_to_touch_ma10"] >= lo) & (ev["bars_to_touch_ma10"] <= hi)]
        angle_q_by_bars[(lo, hi)] = sub["angle_per_bar_ma10"].quantile(Q_PROBS).values

    def angle_label(row):
        b = row["bars_to_touch_ma10"]
        a = row["angle_per_bar_ma10"]
        if pd.isna(a):
            return None
        for lo, hi in BARS_BINS:
            if lo <= b <= hi:
    ...
    for wlabel in WICK_LABELS:
        for lo, hi in BARS_BINS:
            for qlabel in Q_LABELS:
                ...
                rows.append(row)
        # untouched cell for this wick - fwd from cf10 (i+10 close)
        un_cell = untouched[untouched["_wbin"] == wlabel]
        row = {"wick": wlabel, "bars": "UNTCH", "angle": "(fr i+10)"}
        for h in HORIZONS:
            st = cell_stats(un_cell, h, "cf10")
            if h == HORIZONS[-1]:
                row["n"] = st["n"]
            row[f"{h}h_mean"] = st["mean"]
            row[f"{h}h_med"] = st["median"]
    ...
    grid.to_csv(out_dir / "full_grid_summary.csv", index=False, encoding="utf-8")
    ...
    if cfg_path is not None:
        update_config(cfg_path,
                       params={
                           "wick_bins": WICK_BINS,
                           "bars_bins": [list(b) for b in BARS_BINS],
                           "angle_quantile_probs": Q_PROBS,
                       },
                       results_summary={
                           "full_grid_cells": int(len(view)),
                           "touched_ma10": int(len(ev)),
                           "untouched_ma10": int(len(untouched)),
                       })
    return 0
