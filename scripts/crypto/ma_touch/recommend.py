"""Crypto ma_touch recommend — Bitget USDT-M 전 종목 × 5 TF.

실행:
  .venv/Scripts/python.exe -m scripts.crypto.ma_touch.recommend

산출:
  data/cache/crypto/_ma_touch.parquet  (1 row / 종목)
"""
...
def main() -> int:
    ...
    print(f"[Crypto ma_touch] universe={len(syms)} symbols", file=sys.stderr)
