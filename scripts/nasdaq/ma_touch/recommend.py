"""NASDAQ ma_touch recommend — NASDAQ 전 종목 × 5 TF.

실행:
  .venv/Scripts/python.exe -m scripts.nasdaq.ma_touch.recommend

산출:
  data/cache/us/_ma_touch.parquet  (1 row / 종목)
"""
...
ASSET = "us"  # NASDAQ 폴더지만 캐시는 data/cache/us/


def main() -> int:
    sys.stdout.reconfigure(encoding="utf-8")
    syms = discover_universe(ASSET)
    print(f"[NASDAQ ma_touch] universe={len(syms)} symbols", file=sys.stderr)
