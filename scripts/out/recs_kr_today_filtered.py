"""오늘(2026-06-15 Mon) 1.5° + residence 70% 필터 + 약세 진입 통과 종목.

A = passers(Fri) ∩ passers(Mon)
B = passers(Mon) − passers(Fri)
"""
...
def main():
    ...
    fri_pass = set()
    mon_pass = {}   # symbol → (tf, angle, residence)

    for i, sym in enumerate(symbols, 1):
        if best_passing(sym, FRI):
            fri_pass.add(sym)
        m = best_passing(sym, MON)
        if m:
            mon_pass[sym] = m
        if i % 200 == 0:
    ...
    rows = []
    for sym, (tf, angle, res) in mon_pass.items():
        df_full = get_daily(sym)
        if df_full is None or MON not in df_full.index or FRI not in df_full.index:
            continue
        mc = float(df_full.loc[MON, "close"])
        fc = float(df_full.loc[FRI, "close"])
        today_ret = (mc - fc) / fc * 100
        if today_ret > 0:
            continue   # 약세 진입만
        bucket = "A" if sym in fri_pass else "B"
        rows.append({"bucket": bucket, "Symbol": sym, "Name": name_map.get(sym, ""),
                     "tf": tf, "angle_deg": angle, "residence_20": res,
                     "Fri_close": fc, "Mon_close": mc, "today_ret_pct": today_ret})

    df = pd.DataFrame(rows).sort_values(["bucket", "today_ret_pct"]).reset_index(drop=True)
    print(f"\n오늘({MON.date()}) 1.5° + 70% 필터 + 약세 진입 통과: {len(df)}")
    print(f"  A (유지)  : {(df.bucket == 'A').sum()}")
    print(f"  B (신규)  : {(df.bucket == 'B').sum()}")
    ...
    out_csv = Path(__file__).parent / "_recs_kr_today_filtered.csv"
