"""KR 전체 — 상위 TF 정배열 필터 추가 백테스트 (v2).

룰 변경 사항 vs v1:
  + 추가 게이트 E: 진입일 주봉 MA10 > MA20  (큰 흐름 정합)

나머지 룰 동일:
  진입 Z = MA10_d > MA20_d  AND  close > MA20_d  AND  |gap|/ATR20 ≤ 0.5
         AND  fresh (어제 Z 아님)
         AND  ★ 주봉 정배열 ★
  출구: STOP / ★T / TIMEOUT (10봉)
"""
...
def simulate(sym: str, df: pd.DataFrame) -> list[dict]:
    ...
    df["Z_base"] = (df["ma10"] > df["ma20"]) & (df["gap_in_atr"].abs() <= GAP_IN_ATR_MAX) & (df["close"] > df["ma20"])
...
def main():
    ...
    print(f"\n[v2: 주봉 정배열 필터 추가] 총 진입: {len(rdf)} ({rdf['Symbol'].nunique()} 종목)")
    s = rdf["ret_pct"]
    print(f"\n=== 전체 통계 ===")
    ...
    if len(hm):
        print(hm[["entry_date","entry_close","exit_date","exit_close","bars_held","ret_pct","exit_reason"]].to_string(index=False, float_format=lambda v: f"{v:.2f}"))
    else:
        print("  진입 없음 (?)")

    out_csv = Path(__file__).parent / "_backtest_golden_cross_kr_v2.csv"
