"""
_pump_fwd_by_touch_group.py 의 세분화 버전:
  7~20봉은 봉 단위로 분해, 그 외(1~6, 21+)는 합쳐서 컨텍스트.
"""
import sys
...
def assign_group(b: int) -> str:
    if b <= 6: return "00_1~6봉(합)"
    if 7 <= b <= 20: return f"{b:02d}봉"
    return "99_21봉+(합)"
...
def main():
    ...
    med_df = pd.concat([n_per, med.round(2)], axis=1).sort_index()
    mean_df = pd.concat([n_per, mean.round(2)], axis=1).sort_index()

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    med_df.to_csv(OUT_DIR / "pump_fwd_by_touch_bar_median.csv")
    mean_df.to_csv(OUT_DIR / "pump_fwd_by_touch_bar_mean.csv")

    pd.set_option("display.max_columns", None)
    pd.set_option("display.width", 320)

    print("\n=== 표 1. 첫 터치 봉수 별 종가 변화율 중앙값(%) — entry 대비 ===")
    print(med_df.to_string(float_format=lambda x: f"{x:+.2f}"))

    print("\n=== 표 2. 동일하게 평균(%) ===")
    print(mean_df.to_string(float_format=lambda x: f"{x:+.2f}"))

    print(f"\nsaved → {OUT_DIR/'pump_fwd_by_touch_bar_median.csv'}")
    print(f"saved → {OUT_DIR/'pump_fwd_by_touch_bar_mean.csv'}")
