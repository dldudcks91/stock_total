"""
_pump_followthrough.py 의 가지치기 버전:
  + 주봉 SMA20 위에 있는 트리거만 포함

룩어헤드 바이어스 방지:
  - 주봉 MA20 은 직전 완성된 주의 값을 사용 (weekly_ma20.shift(1) 후 1h ts 에 ffill)
"""
import sys
...
def load_with_weekly_ma20(symbol: str) -> pd.DataFrame:
    """1h df with `weekly_ma20` column (lookahead-free)."""
    ...
    # 1d → 1w resample (W-MON, label='left', closed='left' 정책에 맞춤)
    ...
    # 직전 주 ma20 만 사용 (현재 주 진행 중일 때 그 주의 ma20 은 lookahead)
...
def collect_events(symbol: str) -> pd.DataFrame:
    ...
    # 트리거 + 주봉 게이트
    weekly_gate = df["close"] > df["weekly_ma20"]
    mask = (body_pct >= BODY_MIN) & (body_to_range > CLOSE_TO_HIGH_MIN) & weekly_gate
...
def main():
    ...
    ev.to_csv(OUT_DIR / "pump_followthrough_w20gate_events.csv", index=False)
    print(f"\nsaved events → {OUT_DIR/'pump_followthrough_w20gate_events.csv'} ({len(ev):,} rows, {ev['symbol'].nunique()} symbols)")

    print("\n=== 표 1. 전체 집계 (주봉 MA20 위 게이트) ===")
    summary = [
    ...
    per_bar_df.to_csv(OUT_DIR / "pump_followthrough_w20gate_per_bar.csv", index=False)
    print("\n=== 표 2. 봉 인덱스별 forward 통계 ===")
    print(per_bar_df.to_string(index=False, float_format=lambda x: f"{x:+.3f}"))
    print(f"\nsaved per-bar → {OUT_DIR/'pump_followthrough_w20gate_per_bar.csv'}")
